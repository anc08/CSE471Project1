...
CWaveNote::~CWaveNote(void)
{
}

void CWaveNote::XmlLoad(IXMLDOMNode* xml, std::wstring& instrument)
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        else if (name == "wave")
        {
            // Same thing as above
            value.ChangeType(VT_I4);
            m_waveIndex = value.intVal - 1;
        }
    }
}

bool CWaveNote::operator<(const CWaveNote& b)
{
