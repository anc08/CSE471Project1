...
BEGIN_MESSAGE_MAP(CSynthieView, CWnd)
	...
	ON_COMMAND(ID_FILE_LOADWAVFORWAVETABLE, &CSynthieView::OnFileLoadwavforwavetable)
	ON_COMMAND(ID_FILE_CLEARWAVETABLE, &CSynthieView::OnFileClearwavetable)
END_MESSAGE_MAP()
...
void CSynthieView::OnFileLoadwavforwavetable()
{
	static WCHAR BASED_CODE szFilter[] = L"Audio Files (*.wav)|*.wav|All Files (*.*)|*.*||";

	CFileDialog dlg(TRUE, NULL, NULL, 0, szFilter, NULL);
	if (dlg.DoModal() != IDOK)
		return;

	// GIVE WAVE TO M_SYNTHESIZER
	m_synthesizer.AddWaveToTable(dlg.GetPathName());
}


void CSynthieView::OnFileClearwavetable()
{
	m_synthesizer.ClearWaveTable();
}
